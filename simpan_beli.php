...
echo "Nomor Telp harus diisi ! <br/>";
$dataValid="TIDAK" ;
}
if(isset($_COOKIE['keranjang'])){
$barang_pilih = $_COOKIE['keranjang'] ;
}else{
echo "Keranjang Belanja harus diisi ! <br/>";
$dataValid="TIDAK" ;
}
if($dataValid=="TIDAK"){
...
$hasil = mysqli_insert_id($kon) ;
if(!$hasil) {
	echo "Data Custumer Gagal Simpan <br/>" ;
	$simpan = false ;
}
...
if($jumlah <= 1) {
	echo "Tidak Ada Barang Yang Dipilih Yang Dipilih <br/>" ;
	$simpan = false ;
}else{
	foreach($barang_array as $idbarang){
		...
		$sql = "select * form barang where idbarang = $idbarang = $idbarang " ;
		$hasil = mysqli_query($kon, $sql);
		if(!$hasil){
			echo "Barang Tidak Ada <be/>" ;
			$simpan = false ;
			break;
		}else{	
...
haeder("location : bukti_beli.php?idhjual=$idhjual") ;
